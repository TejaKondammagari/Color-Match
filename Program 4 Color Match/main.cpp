...
//
// Author: Tejaswi Reddy Kondammagari, Damian Charczuk
// Lab: Tues 9 am
...
//---------------------------------------------------------------------------------------
// Set Color of a square if isCaptured of the square is true.
void setColor(sf::Color c, Square squaresArray[], int arraySize)
{
    for(int i=0; i<arraySize; i++){
        Square s = squaresArray[i];
        if(s.getIsCaptured()) // check if isCaptured for the square is true
        {
            squaresArray[i].setColor(c); // Set color of the square
        }
    }
}

// Check if the color of the square is the color of the provided square. If it is same, set isCaptured as true.
void checkIsCaptured(Square squaresArray[], int i, int n)
{
    if(squaresArray[0].getColor() == squaresArray[n].getColor()) // Compare the colors of provided squares
    {
        squaresArray[n].setIsCaptured(true); // Set isCaptured of the square to true
        squaresArray[n].setColor(squaresArray[0].getColor()); // Set the color of the square to the provided square
    }
}

// Check if which squares in the squaresArray are under the captured region and set isCaptured value to true
void checkCapturedSquares(Square squaresArray[], int arraySize, int level)
{
    for(int i=0; i<arraySize; i++){
        sf::Color color = squaresArray[i].getColor(); // Get color of the ith square
        int mod = i%level;
        
        // Check and set isCaptured values of the adjacent squares if the value of isCaptured for the ith square is true
        if(squaresArray[i].getIsCaptured() && i!=arraySize){
            if(mod == (level-1))
            {
                checkIsCaptured(squaresArray, i, i+level);
                checkIsCaptured(squaresArray, i, i-level);
                checkIsCaptured(squaresArray, i, i-1);
            }
            else if(i>(arraySize-level))
            {
                checkIsCaptured(squaresArray, i, i+1);
                checkIsCaptured(squaresArray, i, i-level);
                checkIsCaptured(squaresArray, i, i-1);
...
int main()
{
    ...
    int par = 8; // initialize par value
    int steps = 0; // initialize steps to increment for Moves
    ...
    while (window.isOpen())
    {
        ...
        bool levelUp = true;
        bool reset = false;
        sf::Color c = squaresArray[0].getColor(); // Initialize c to the color of 0th square in squaresArray
        
        // check if all the squares are captured or of same color
        for( int i=0; i<arraySize; i++) {
            if(squaresArray[i].getColor() != c){
                levelUp = false;
                reset = false;
                break;
            }
        ...
        if(levelUp){
            settingUpBoard = true;
            level = level + 1;
            if(level == 25) // if level is greater than 24, close the window.
            {
                window.close();
            }
        ...
        int size = WindowXSize/level; // Initialize size based on the level
        ...
        if( settingUpBoard == true) {
            ...
            {
                y=0;
                while(y<WindowXSize && (y+size <= WindowXSize))
                {
                    char name[ 81]; // prints the empty value on squares
                    squaresArray[i] = Square(size, x, y, getRandomColor(), true, name); // initializes a sqaure to random color
                    i++;
                    y = y+size;
                }
                x = x+size;
            ...
            squaresArray[0].setIsCaptured(true);  // Check and Set isCaptured for Squares
            checkCapturedSquares(squaresArray, arraySize, level); // check and set captured squares
        ...
        for( int i=0; i<arraySize; i++) {
            ...
            sprintf( aString, "(par) %d Move %d", par, steps);  // Make a string to display par and steps
        }
        ...
        if (sf::Mouse::isButtonPressed(sf::Mouse::Left) && !mouseIsAlreadyPressed)
        {
            ...
            int selectedSquareNum = 0;
            sf::Color selColor;
            // if the mouse click is out of board region set the selectedColor to Black
            if(localPosition.x > 400 || localPosition.y > 400){
                selColor = sf::Color::Black;
            }
            else{
                // if the mouse click is on board region, get the number of the square
                for(int i=0;i<arraySize;i++)
                {
                    int Xposition = squaresArray[i].getXPosition();
                    int Yposition = squaresArray[i].getYPosition();
                    if(localPosition.x >= Xposition && localPosition.x <= (Xposition+size) &&
                       localPosition.y >= Yposition && localPosition.y <= (Yposition+size)){
                        selectedSquareNum = i;
                        break;
                    }
                }
                selColor = squaresArray[selectedSquareNum].getColor();
            }
            ...
            for(int i=0; i<arraySize; i++){
                ...
                {
                    squaresArray[i].setColor(selColor);
                }
            }
            
            checkCapturedSquares(squaresArray, arraySize, level);
        }
